...
def update_data():
    ...
    for hospital in df_data.hospital.unique():
        ...
        for bed_type in ('Oxygen Bed', 'Isolation Bed','Ventilator Bed'):
            ...
            index_ = df_data[(df_data['hospital']==hospital) & (df_data['bed_type']==bed_type) ].index

            occupied = df_p1_b[df_p1_b['alloted']==True]['patient_id'].count()
            queue = df_p1_b[df_p1_b['in_queue']==True]['patient_id'].count()
            total = df_data.iloc[index_]['total']
            vacant = total - occupied
            df_data.loc[index_,('occupied','vacant','queue')] =[occupied, vacant, queue]

        df_p1_q = df_p1[df_p1['in_queue']==True]
        queue_not_decided = df_p1_q[(df_p1_q['bed_type']=='no_bed_type')]['patient_id'].count()
        df_data.loc[(df_data['hospital']==hospital) & (df_data['bed_type']==bed_type),'queue'] = queue_not_decided
...
def data(request: Request):
    ...
    data = data.groupby(['hospital','bed_type']).apply(lambda x: x[['total','occupied','vacant','queue']].to_dict('list')).reset_index()
    data_json = data.groupby('hospital').apply(lambda x: dict(zip(x['bed_type'], x[0]))).to_json(orient="index")
